...
## 1. Transaction Type Duplication (Critical)
...
This could be driven by configuration or a factory pattern, reducing duplication to a single source of truth.

## 2. Account.statement() is 160 Lines (Critical)
...
Should be split into separate classes or methods with clear single responsibilities.
...
1. **Exception typo fix** (#8) — Quick win, low risk warm-up
2. **Transaction duplication** (#1) — Highest line-count reduction
3. **Account.statement() split** (#2) — Improves cohesion
